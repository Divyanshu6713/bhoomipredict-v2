  ...
  return (
    <div className="space-y-5">
      ...
      {s.retrainRecommended && (
        <p className="flex items-start gap-2 rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-2.5 text-sm text-ink-2">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        ...
        <Card>
                ...
                {(drift.data?.features ?? []).map((f) => (
                  <tr key={f.feature}>
                    ...
                    <td className="px-5 py-1.5">
                      <Badge className={cn(f.status === 'significant' ? 'border-red-500/25 bg-red-500/10 text-red-600' : f.status === 'watch' ? 'border-amber-500/25 bg-amber-500/10 text-amber-700' : 'border-emerald-500/25 bg-emerald-500/10 text-emerald-700')}>{f.status}</Badge>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {drift.data && <p className="border-t border-line px-5 py-2.5 text-xs text-ink-3">PSI &lt; 0.10 stable · 0.10–0.25 watch · &gt; 0.25 significant. Stage-mix PSI {drift.data.stageMix.psi}. {drift.data.note}</p>}
        </Card>
      </section>
...
function LoopStep({ icon: Icon, title, value, note, tone }: { icon: typeof GitBranch; title: string; value: string; note: string; tone?: 'warn' | 'bad' }) {
  return (
    <Card className={cn('p-4', tone === 'warn' && 'border-amber-500/40', tone === 'bad' && 'border-red-500/40')}>
      <p className="flex items-center gap-1.5 label-xs">
        <Icon className="h-3.5 w-3.5" /> {title}
      </p>
      <p className="mt-2 truncate text-xl font-semibold leading-none text-ink num">{value}</p>
      <p className="mt-1.5 text-xs leading-snug text-ink-3">{note}</p>
    </Card>
  );
}
  ...
  return (
    <Card>
      ...
      <div className="space-y-3 px-5 pb-5">
        {a.outcomes === 0 ? (
          <p className="text-xs text-ink-3">No outcomes recorded yet. Record one from a case page, upload a CSV, or advance the simulation clock below.</p>
        ) : (
          <>
            <KeyValue
              columns={3}
              rows={[
                { label: 'Live ROC-AUC', value: f3(a.rocAuc), hint: s.live.offlineTest ? `offline test ${s.live.offlineTest.rocAuc.toFixed(3)}` : undefined, tone: s.live.rocAucDropVsTest !== null && s.live.rocAucDropVsTest > 0.05 ? 'text-red-600' : undefined },
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-ink-3">Bar = mean predicted probability · orange mark = observed delay rate. {s.live.note}</p>
            </div>
          </>
        )}
      </div>
    ...
    <Card className="xl:col-span-2">
      ...
      <div className="grid gap-3 px-5 pb-5 md:grid-cols-3">
        <div className="rounded-lg border border-line bg-surface-2 p-3.5">
          <p className="text-sm font-semibold text-ink">1 · Officer records a milestone</p>
          <p className="mt-1 text-xs text-ink-3">On any open case page, a Land Acquisition Officer or administrator records the date the milestone was achieved (or that it is still pending past 30 days).</p>
          <Link to="/cases?status=open" className="mt-2 inline-block text-xs font-semibold text-brand hover:underline">
            Open cases →
          </Link>
        </div>
        <div className="rounded-lg border border-line bg-surface-2 p-3.5">
          <p className="text-sm font-semibold text-ink">2 · CSV or API ingestion</p>
          <p className="mt-1 text-xs text-ink-3">Bulk outcomes from a state system: every row validated; committed only if all pass. Systems can also POST /api/v1/outcomes with an API key.</p>
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            <label className={cn('inline-flex cursor-pointer items-center gap-1 rounded-lg border border-line px-2.5 py-1 text-xs font-semibold text-ink-2 hover:border-brand/50', !canRecord && 'pointer-events-none opacity-50')}>
          ...
          {csvResult && (
            <div className="mt-2 text-xs text-ink-2">
              ...
              {csvResult.errors.slice(0, 4).map((e) => (
                <p key={e.row} className="font-mono text-xs text-red-600">
                  row {e.row}: {e.error}
                </p>
              ))}
            </div>
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/[0.06] p-3.5">
          <p className="text-sm font-semibold text-ink">3 · Simulation clock (demo)</p>
          ...
          <div className="mt-2 flex items-center gap-1.5">
            <select value={days} onChange={(e) => setDays(Number(e.target.value))} className="h-8 rounded-lg border border-line bg-surface px-2 text-xs">
              {[30, 60, 90, 180].map((d) => (
        ...
        {msg && <p className="md:col-span-3 text-xs text-ink-2">{msg}</p>}
      </div>
    </Card>
  );
}
...
function Retraining({ canRetrain, s, onChange }: { canRetrain: boolean; s: LearningStatus; onChange: () => void }) {
  ...
  return (
    <Card>
      ...
      <div className="space-y-3 px-5 pb-5">
        <p className="text-xs text-ink-3">{s.environment.ok ? `Environment: ${s.environment.detail}` : `Environment unavailable: ${s.environment.detail}`}</p>
        <Button
        ...
        <label className="flex items-center justify-between gap-2 rounded-lg border border-line bg-surface-2 px-3 py-2 text-xs">
          <span>
            <span className="font-semibold text-ink">Automatic retraining</span>
            <span className="block text-xs text-ink-3">when at least</span>
          </span>
          <input type="number" min={100} step={100} value={minOutcomes} onChange={(e) => setMinOutcomes(Number(e.target.value))} className="h-8 w-20 rounded-lg border border-line bg-surface px-2 text-right text-xs num" disabled={!canRetrain} />
          <span className="text-xs text-ink-3">outcomes</span>
          <input
            type="checkbox"
        ...
        {job.status !== 'idle' && (
          <div className="space-y-1">
            <p className="text-xs font-semibold text-ink">
              {job.kind ?? 'job'} {job.id} · <span className={cn(job.status === 'rejected' && 'text-amber-700', job.status === 'failed' && 'text-red-600', job.status === 'succeeded' && 'text-emerald-700')}>{job.status}</span>
            </p>
            {job.steps.map((st) => (
              <p key={st.name} className="text-xs text-ink-2">
                <Badge className="mr-1">{st.status}</Badge>
                {st.name}
              </p>
            ))}
            {job.result?.gate?.reason && <p className="text-xs text-ink-2">Gate: {job.result.gate.reason}</p>}
            <pre className="max-h-32 overflow-auto rounded-lg bg-navy-900 p-2 font-mono text-2xs text-white/80">{job.log.slice(-14).join('\n')}</pre>
          </div>
        )}
      </div>
    </Card>
          ...
          <tbody className="divide-y divide-line">
            {versions.map((v) => (
              <tr key={v.id} className={cn(v.id === champion && 'bg-brand/[0.04]')}>
                <td className="px-5 py-2 font-mono text-xs font-semibold text-ink">{v.id}</td>
                <td className="px-3 py-2">
                  <Badge className={cn(v.status === 'champion' ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-700' : v.status === 'rejected' ? 'border-amber-500/25 bg-amber-500/10 text-amber-700' : 'border-line bg-surface-2 text-ink-3')}>
                    {v.status === 'champion' ? <CheckCircle2 className="h-3 w-3" /> : v.status === 'rejected' ? <XCircle className="h-3 w-3" /> : null} {v.status}
                  </Badge>
                </td>
                <td className="px-3 py-2 text-ink-2">
                  {formatDate(v.createdAt)}
                  <span className="block text-xs text-ink-3">{v.triggeredBy}</span>
                </td>
                ...
                <td className="max-w-[340px] px-3 py-2 text-xs leading-snug text-ink-2">{v.gate.reason}</td>
