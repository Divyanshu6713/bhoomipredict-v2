...
const TONE: Record<StageStatus, { ring: string; text: string; line: string }> = {
  COMPLETED: { ring: 'border-emerald-500 bg-emerald-500 text-white', text: 'text-emerald-700 dark:text-emerald-300', line: 'bg-emerald-500' },
  IN_PROGRESS: { ring: 'border-sky-500 bg-sky-500 text-white', text: 'text-sky-700 dark:text-sky-300', line: 'bg-sky-500' },
  DELAYED: { ring: 'border-amber-500 bg-amber-500 text-white', text: 'text-amber-700 dark:text-amber-400', line: 'bg-amber-500' },
  BLOCKED: { ring: 'border-red-500 bg-red-500 text-white', text: 'text-red-700 dark:text-red-300', line: 'bg-red-500' },
    ...
    <div className="px-5 pb-5">
          ...
          {stages.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                {s.status === 'COMPLETED' && s.delayDays > 0 && <p className="text-2xs font-semibold text-amber-700 dark:text-amber-400 num">+{s.delayDays}d late</p>}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-line pt-3">
        {(['COMPLETED', 'IN_PROGRESS', 'DELAYED', 'BLOCKED', 'PENDING'] as StageStatus[]).map((st) => (
          <span key={st} className="flex items-center gap-1.5 text-xs text-ink-2">
