  ...
  return (
    <Card>
      <CardHeader
        ...
        action={
          <Link to="/data-sources" className="text-xs font-semibold text-brand hover:underline">
            Data sources
          </Link>
        }
      />
      <div className="px-5 pb-5">
        {view.error && <p className="text-xs text-red-600">{view.error.message}</p>}
            ...
            {(Object.entries(view.data.sections) as Array<[string, Envelope<Record<string, unknown>>]>).map(([key, env]) => (
              <div key={key} className="rounded-lg border border-line bg-surface-2 p-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-ink">{SECTION_LABEL[key] ?? key}</p>
                  {env.provenance && <ProvenanceBadge mode={env.provenance.mode} compact />}
                </div>
                {env.error ? (
                  <p className="mt-2 text-xs text-red-600">{env.error}</p>
                ) : env.data ? (
                ...
                {env.provenance && <p className="mt-2 border-t border-line pt-1.5 text-xs text-ink-3">{env.provenance.source}</p>}
              </div>
            ))}
          </div>
        )}
