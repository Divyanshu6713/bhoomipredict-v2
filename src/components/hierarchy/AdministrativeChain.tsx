      ...
      {position.chain.map((row, i) => {
        ...
        return (
          <li key={`${row.level}-${i}`} className="relative flex gap-3 pb-3 last:pb-0">
            ...
            <span
              className={cn(
                'relative z-[1] grid h-7 w-7 shrink-0 place-items-center rounded-lg border',
                row.all ? (dark ? 'border-white/10 bg-white/[0.03] text-white/35' : 'border-line bg-surface-2 text-ink-3') : dark ? 'border-brand/40 bg-brand/20 text-[#9DBBFF]' : 'border-brand/25 bg-brand/10 text-brand',
              )}
            >
              <Icon className="h-3.5 w-3.5" />
            </span>
            <div className="min-w-0 pt-0.5">
              <p className={cn('text-[10px] font-bold uppercase tracking-[0.1em]', dark ? 'text-white/40' : 'text-ink-3')}>{row.label}</p>
              <p className={cn('text-[13px] font-semibold leading-snug', row.all ? (dark ? 'text-white/45' : 'text-ink-3') : dark ? 'text-white' : 'text-ink')}>
                {row.value}
                {row.code && <span className={cn('ml-1.5 font-mono text-[10.5px] font-medium', dark ? 'text-white/40' : 'text-ink-3')}>{row.code}</span>}
              </p>
            </div>
          </li>
        );
...
export function PositionBreadcrumb({ position, className }: { position: Position; className?: string }) {
  ...
  return (
    <p className={cn('flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11.5px] text-ink-3', className)}>
      {rows.map((r, i) => (
        <span key={`${r.level}-${i}`} className="flex items-center gap-1.5">
          {i > 0 && <span className="text-ink-3/60">›</span>}
          <span className={cn(i === rows.length - 1 && 'font-semibold text-ink-2')}>{r.level === 'ministry' || r.level === 'organisation' || r.level === 'department' ? shorten(r.value) : r.value}</span>
        </span>
      ))}
    </p>
  );
