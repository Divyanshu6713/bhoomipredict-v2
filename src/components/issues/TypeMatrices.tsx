...
function Legend({ items }: { items: Array<[string, string]> }) {
  return (
    <div className="flex flex-wrap gap-x-4 gap-y-1 px-5 pb-3 text-[11px] text-ink-3">
        ...
        <table className="w-full min-w-[980px] text-[11px]">
          <thead>
            <tr className="border-b border-line bg-surface-2">
              <th className="sticky left-0 z-[1] bg-surface-2 px-3 py-2 text-left font-bold uppercase tracking-wider text-ink-3">Type</th>
              {codes.map((c) => (
            ...
            {matrix.map((row) => (
              <tr key={row.projectType} className="border-b border-line/60 last:border-0">
                ...
                {row.dependencies.map((d) => (
                  <td key={d.code} title={d.applicability === 'conditional' ? `${d.label}: ${d.when.join(' — or — ')}` : `${d.label}: ${d.applicability}`} className={cn('px-1 py-1.5 text-center text-[13px]', d.applicability === 'always' ? 'text-brand' : d.applicability === 'conditional' ? 'text-amber-600 dark:text-amber-400' : 'text-ink-3/50')}>
          ...
          <tbody>
            {matrix.map((row) => (
              <tr key={row.projectType} className="border-b border-line/60 last:border-0" title={row.typicalDependencies ?? undefined}>
                <td className="sticky left-0 z-[1] whitespace-nowrap bg-surface px-3 py-1.5 font-semibold text-ink">{row.projectType}</td>
                {row.issues.map((i) => (
                  <td key={i.id} className={cn('px-1 py-1.5 text-center text-[13px]', i.exposure === 'core' ? 'text-rose-600 dark:text-rose-400' : i.exposure === 'possible' ? 'text-ink-2' : 'text-ink-3/50')}>
