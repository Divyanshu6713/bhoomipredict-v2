...
export const ISSUE_STATUS_CLASS: Record<IssueStatus, { chip: string; dot: string }> = {
  active: { chip: 'border-red-500/30 bg-red-500/10 text-red-700 dark:text-red-400', dot: 'bg-red-500' },
                ...
                <button onClick={() => setOpenId(open ? null : issue.id)} className="flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-surface-2" aria-expanded={open}>
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex flex-wrap items-center gap-1.5 text-sm font-semibold text-ink">
                      {issue.label}
                      {issue.core && <span className="rounded border border-brand/25 bg-brand/[0.07] px-1 py-px text-2xs font-medium text-brand">key for type</span>}
                    </span>
                    <span className="block truncate text-xs text-ink-3">{issue.evidence[0]}</span>
                  </span>
                  <span className={cn('hidden shrink-0 rounded-md border px-1.5 py-0.5 text-xs font-medium sm:inline', ISSUE_STATUS_CLASS[issue.status].chip)}>{issue.statusLabel}</span>
                  <ChevronDown className={cn('h-3.5 w-3.5 shrink-0 text-ink-3 transition-transform', open && 'rotate-180')} />
                </button>
                {open && (
                  <div className="space-y-2 bg-surface-2 px-3 pb-3 pl-8 pt-1 text-xs text-ink-2">
          ...
          <div className="border-t border-line pt-3">
            ...
            <ul className="space-y-1">
              {profile.excluded.map((e) => (
                <li key={e.id} className="text-xs text-ink-3">
